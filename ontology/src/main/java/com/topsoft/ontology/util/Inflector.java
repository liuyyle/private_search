...
public class Inflector {
    ...
    * @return the pluralized form of the term, only all words need to be singularized
    */
   public String singularizeAllWords( Object term ) {
       if (term == null) return null;
       String termStr = term.toString().trim();
       if (termStr.length() == 0) return termStr;
    ...
    public String singularizeTerm( Object term ) {
        ...
        if(termList.size() > 1) 
        {
        	...
        	for(int i = 0; i<termList.size(); i++) {
                if(i == 0)
                	termStr += singularizeAllWords(termList.get(i));
                else if(i == termList.size()-1)
                	termStr += " and " +singularizeAllWords(termList.get(i));
                else
                	termStr += ", " +singularizeAllWords(termList.get(i));
            }
        }else {
        	termStr = singularizeAllWords(termStr);
        }
